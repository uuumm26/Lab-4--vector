...
{
	if (len < 1) {
		throw runtime_error("Длина больше нуля!!!");
	}
	data = new T[len];
	fill_n(data, len, 0);
}
...
{
	if (size() != p.size())
		throw runtime_error("Размеры должны быть одинаковыми");
...
{
	if (size() != p.size())
		throw runtime_error("Размер умножаемых векторов должен быть одинаковым");
	TVector<T> result(size());
	for (int i = 0; i < result.size(); i++) {
		result[i] = (*this)[i] / p[i];
	}
...
inline TVector<T> TVector<T>::operator*(const TMatrix<T>& matrix) const
{
	...
	{
		throw runtime_error("Разные размеры");
	}
	TVector<T> result(size());
	for (int i = 0; i < size(); i++)
